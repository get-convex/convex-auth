...
function ValidateEmailWithCode({ emailCode }: { emailCode: string }) {
  ...
  switch (state.userError.error) {
    ...
    case "INVALID_CHALLENGE":
      return (
        <ValidateEmailError message="This link is no longer valid. It may have expired or already been used." />
      );
    case "INCORRECT_CODE":
      return (
        <ValidateEmailError message="This is not the latest link. Open the newest email we sent you." />
      );
    case "EMAIL_TAKEN":
      return (
        <ValidateEmailError message="Another account validated this email address first." />
      );
    case "OTHER_ERROR":
      console.error("Validation failed:", state.userError.cause);
      return (
        <ValidateEmailError message="Something went wrong. Reload the page to try again." />
      );
    default:
      state.userError satisfies never;
      return (
        <ValidateEmailError message={"Unknown error: " + state.userError} />
      );
  }
}

function ValidateEmailError({ message }: { message: string }) {
  return (
    <>

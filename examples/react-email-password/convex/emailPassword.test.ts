...
import { normalizeEmail } from "@convex-dev/auth/providers/email-password/validation";
 ...
 * cast. We use it to seed component state that only `startValidation` writes
...
/** Seed a pending validation, hashing the code + secret like production. */
...
describe("signUp", () => {
  ...
  // checkStartValidation, which reads the client IP).
...
describe("completeSignUp", () => {
  test("validates the email and signs the user in", async () => {
    ...
    // pending validation (no verified email yet).
    const userId = await t.run(
...
describe("startChangeEmail", () => {
  ...
  // TODO: enable when convex-test supports ctx.meta (the happy path reaches
  // startValidation, which reads the client IP).
...
describe("getValidationStatus", () => {
  test("reports pending for a live link and invalid otherwise", async () => {
    ...
    expect(
      await t.query(api.auth.getValidationStatus, {
        code: "code1",
        secret: "secret1",
      }),

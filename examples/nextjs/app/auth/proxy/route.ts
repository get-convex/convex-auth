...
// (/auth/proxy?path=/api/mutation, ?path=/api/action) for every provider listed
// in `signIn` in serverAuth.ts, forwarding each call to the deployment and
// moving the minted refresh token into an httpOnly cookie on the way back.
//

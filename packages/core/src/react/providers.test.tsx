...
describe("useAuthClientValue", () => {
  test("throws when used outside a provider", () => {
    expect(() => renderHook(() => useAuthClientValue("greeting"))).toThrow(
  ...
  test("returns undefined for an unregistered key", () => {
    ...
    const { result } = renderHook(() => useAuthClientValue("missing"), {
  ...
  test("reads a value seeded before render and re-renders on set", () => {
    // Setups run while the client is constructed, so their values must be
    // visible on the app's very first render.
    const client = makeClient();
    client.store.set("greeting", "hello");

    const { result } = renderHook(
      () => useAuthClientValue<string>("greeting"),
      { wrapper: wrapperFor(client) },
    );
    expect(result.current).toBe("hello");

    act(() => {
      client.store.set("greeting", "hi again");
    });

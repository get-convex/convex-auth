 ...
 * `useAuthActions`).
 *
 * The email flows split in two: a *start* call sends a validation link and
...
import { NamespacedStorage, defaultStorage } from "../../browser/storage";
import type {
  SignUpResult,
...
/**
 * A failure the client produces when a validation link is opened in a
...
type GetValidationStatusQuery = FunctionReference<
  "query",
  "public",
  { code: string; secret: string },
  ValidationStatus
...
/**
 * What a landing page shows for a validation link:
 *
 ...
 */
export type UseValidationStatusResult =
  ValidationStatus | { status: "missingSecret" } | undefined;

/**
 * Subscribe to the state of a validation link, for landing pages: read the
 * flow's secret from storage, then run the backend's `getValidationStatus`
 * query with it.
 *
 * @param statusQuery The app's `getValidationStatus` query reference.
...
export function useValidationStatus(
  statusQuery: GetValidationStatusQuery,

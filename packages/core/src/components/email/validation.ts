...
/**
 * How a verified email address got verified.
 *
 * - `self`: the user proved ownership of the address through a challenge.
 *
 * TODO: add sources for addresses that a trusted third party verified (for
 * example an OAuth provider).
 */
export const vEmailSource = v.union(v.object({ kind: v.literal("self") }));
export type EmailSource = Infer<typeof vEmailSource>;

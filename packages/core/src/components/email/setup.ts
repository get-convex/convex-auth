...
/**
 * How the recipe sends emails: validation links (through the email
...
export type EmailPasswordUrls = {
  ...
  signUp: string;
  /** Landing page for the change-email validation link. */
  changeEmail: string;
...
export type EmailPasswordOptions = {
   ...
   * to track verified emails and to run the validation flows.
   */
...
const signUpResult = v.union(
  ...
  v.object({
    ...
    userError: v.union(startValidationUserError, setPasswordUserError),
  }),
);
...
const startRecoveryResult = v.union(
  ...
  v.object({
    success: v.literal(false),
    userError: startValidationUserError,
  }),
);
...
const completeRecoveryResult = v.union(
  ...
  v.object({
    success: v.literal(false),
    userError: v.union(completeValidationUserError, setPasswordUserError),
  }),
);
 ...
 *   signs in only after they open the validation link. Wire it up in
 *   `convex/auth.ts`:
 *
 * ```ts
...
export function setupEmailPassword<UsersTable extends string>(
  ...
  /** The sender config the email component's `startValidation` accepts. */
  ...
  const primaryEmail = async (
    ...
    const emails = await ctx.runQuery(component.public.getEmails, {
      userId,
    });
  ...
  return {
    ...
    attachUserCallback(
      ...
      return {
        ...
        signUp: authMutation({
          ...
          handler: async (ctx, { email, password }): Promise<SignUpResult> => {
            ...
            const check = await ctx.runMutation(
              component.public.checkStartValidation,
              { email },
            );
            if (!check.ok) {
              return {
         ...
         * a validation link to the new address; the change happens in
         * `completeChangeEmail`.
         */

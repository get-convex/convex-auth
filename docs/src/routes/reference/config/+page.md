...
| `path`                            | `string`                                            | `"/auth"` | HTTP path where the app-owned auth protocol routes are mounted. Provider callbacks, the JWT issuer, OAuth discovery, and `auth.request.add(http)` all use this path. |

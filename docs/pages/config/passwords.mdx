...
<Callout type="info">
  The `signIn` function can return structured error information for displaying
  feedback to users. See [Error Handling](/error-handling) for details.
</Callout>
...
The async `signIn` function returns an object with a `signingIn` field
indicating whether the sign-in was immediately successful, and an optional
`error` field containing a structured error code if
[error handling](/error-handling) is configured. In the example below we don't
check the result, as we assume that the whole `SignIn` component will be
unmounted when the user is signed-in.

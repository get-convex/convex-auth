...
<Callout type="info">
  You can handle errors like invalid credentials using structured error codes.
...
indicating whether the sign-in was immediately successful. If
[error handling](/error-handling) is configured, the object also includes an
`error` field with a structured error code. In the example below we don't check
the result, as we assume that the whole `SignIn` component will be unmounted

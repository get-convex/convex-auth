...
Google Cloud project. If you are working with an existing project, it's possible that some of
the steps below won't apply.
...
Copy the **Client ID** and set it in environment vars.

Copy the **Client secret** and set it in environment vars.

Open a terminal to your Convex application directory. Copy the **Client ID**
value from the configuration page and supply it as the last argument to the
following command (run in your terminal):

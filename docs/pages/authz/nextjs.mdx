...
If you have custom OAuth flows that use the `code` query parameter (like Figma
OAuth), you can prevent Convex Auth from handling those codes by providing a
`shouldHandleCode` callback:
...
The callback receives the request object and should return `true` if Convex Auth
should handle the code parameter, or `false` if it should be left alone for your
custom OAuth flow to handle.

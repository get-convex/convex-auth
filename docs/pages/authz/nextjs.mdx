...
Convex queries are safe to call from components and `GET` Route Handlers.
...
authentication state available only to same-origin requests by default.

Convex Auth is not special here, and the same applies to most other

...
You can enrich JWT tokens with app-specific data using the
[`jwt.customClaims`](/api_reference/server#jwtcustomclaims) callback. The
claims you return are spread into the JWT payload and become available via
`ctx.auth.getUserIdentity()` in your backend functions.
...
You can then read these claims in any query, mutation, or action:
...
The callback is called every time a token is generated, including on token
refresh.

<Callout type="warning">
  Returning a reserved claim name (`sub`, `iss`, `aud`, `iat`, `exp`, `nbf`,
  `jti`) will throw an error.
</Callout>

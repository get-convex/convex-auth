...
1. Each provider is determined "trusted" or not, based on whether the email
   ...
   2. OAuth providers are trusted by default, since most popular OAuth providers
      today enforce email verification befor the user can use them. You can make
      any OAuth provider untrusted by setting the
...
2. If a trusted method is used, and there is a single existing user document
   ...
   - If there are multiple user documents with the same email address, then
     account linking is not performed. This can only happen if your app uses
     multiple untrusted authentication methods.

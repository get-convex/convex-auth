import { Steps } from "nextra/components";
import { Tabs } from "nextra-theme-docs";

# Set Up Convex Auth

This guide assumes you have a working Convex app. If not, follow our React or
Next.js [quickstart](https://docs.convex.dev/quickstarts) first.

<Steps>
### Install the NPM library
 
```sh
npm install @xixixao/convex-auth @auth/core
```

This also installs `@auth/core`, which you will use during provider
configuration later.

### Run the initialization script

```sh
npx @xixixao/convex-auth
```

This sets up your project for authenticating via the library.

Alternatively you can perform these steps manually:
[Manual Setup](/setup/manual)

### Add authentication tables to your schema

Convex Auth assumes you have several tables set up with specific indexes.

You can add these tables to your
[schema](https://docs.convex.dev/database/schemas) via the `tables` export:

```ts filename="convex/schema.ts"
import { defineSchema } from "convex/server";
import { v } from "convex/values";
import { tables } from "@xixixao/convex-auth";

const schema = defineEntSchema({
  ...tables,
  // Your other tables...
});

export default schema;
```

### Set up the React provider

Convex Auth currently only supports authenticating from React client components.

<Tabs items={['Vite', 'Next.js']}>
<Tabs.Tab>

Replace `ConvexProvider` from `convex/react` with `ConvexAuthProvider` from
`@xixixao/convex-auth/react`, and pass it the `providers` returned by
`convexAuth()` in `convex/auth.ts`:

```ts filename="src/main.tsx" {1,7,13,15}
import { ConvexAuthProvider } from "@xixixao/convex-auth/react";
import React from "react";
import ReactDOM from "react-dom/client";
import { ConvexReactClient } from "convex/react";
import App from "./App.tsx";
import "./index.css";
import { providers } from "../convex/auth";

const convex = new ConvexReactClient(import.meta.env.VITE_CONVEX_URL as string);

ReactDOM.createRoot(document.getElementById("root")!).render(
  <React.StrictMode>
    <ConvexAuthProvider client={convex} providers={providers}>
      <App />
    </ConvexAuthProvider>
  </React.StrictMode>,
);
```

</Tabs.Tab>
<Tabs.Tab>

Replace `ConvexProvider` from `convex/react` with `ConvexAuthProvider` from
`@xixixao/convex-auth/react`, and pass it the `providers` returned by
`convexAuth()` in `convex/auth.ts`:

```ts filename="app/ConvexClientProvider.tsx" {2,3,11,13}
"use client";
import { ConvexAuthProvider } from "@xixixao/convex-auth/react";
import { providers } from "@/convex/auth";
import { ConvexReactClient } from "convex/react";
import { ReactNode } from "react";

const convex = new ConvexReactClient(process.env.NEXT_PUBLIC_CONVEX_URL!);

export function ConvexClientProvider({ children }: { children: ReactNode }) {
  return (
    <ConvexAuthProvider client={convex} providers={providers}>
      {children}
    </ConvexAuthProvider>
  );
}
```

</Tabs.Tab>
</Tabs>

</Steps>

The initial setup is done. Next you'll
[choose and configure authentication methods](/config).
